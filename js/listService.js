...
(function () {
    ...
    function listService($localStorage) {
        ...
        ls.updateDate = updateDate;
        ls.deleteAllSelected = deleteAllSelected;
        ls.updateList = updateList;
        ...
        function deleteAllSelected(list) {
            if (list == 'all'){
                for (var i = 0; i < ls.storage.doList.length; i++){
                    if (ls.storage.doList[i].done == true) {
                ...
                for (var i = 0; i < ls.storage.doList.length; i++){
                    if (ls.storage.doList[i].done == true && ls.storage.doList[i].type == ls.currentList) {
                        ls.storage.doList.splice(i, 1);
                    }
                }
            }
        ...
        function archiveChecked(){
            for (var i = 0; i < ls.storage.doList.length; i++){
                (ls.storage.doList[i].done) ? ls.storage.doList[i].archive = true : ls.storage.doList[i].archive = false;
            }
        }
        ...
        function deleteAllInList(num){
            for (var i = 0; i < ls.storage.doList.length; i++){
                if (ls.storage.doList[i].type == num && ls.storage.doList[i].archive == false){
                    ls.storage.doList.splice(i, 1);
                    i--
                }
            }
